...
export const ROUTE_REGISTRY = {
  ...
  // Spend breaker: the ceiling written here is what checkCostCeilingGuard()
  // (packages/runtime/src/loop/guards.ts) reads from the `cost_ceiling_cents`
  // state key to auto-pause the loop. Without this route the guard is enforced
  // but unsettable.
  "/cost-ceiling": {
    GET: async (ctx: RouteCtx) => {
      const result = await handleGetCostCeiling(asDrizzleDb(ctx.db));
      return Response.json(result, { headers: ctx.corsHeaders });
    },
    POST: async (ctx: RouteCtx) => {
      const result = await handleSetCostCeiling(asDrizzleDb(ctx.db), ctx.body);
      return Response.json(result, { headers: ctx.corsHeaders });
    },
  },
  "/streaming": {
  ...
  "/bulk-archive": {
    POST: async (ctx: RouteCtx) => {
      ...
      try {
        ...
        // Configured human name (D1 `human_name`, default "User") for the
        // summarization prompt's transcript labels. (Cast: ctx.db carries the
        // same pre-existing D1Database-vs-Drizzle annotation mismatch as every
        // sibling getState call in this file.)
        const humanName =
          (
            (await getState(
              ctx.db as unknown as Parameters<typeof getState>[0],
              "human_name",
            )) || ""
          ).trim() || "User";
        ...
        for (let chunk = 0; chunk < chunksToProcess; chunk++) {
          ...
          const entriesText = entries
            .map((h: any) => {
              ...
              switch (h.type) {
                case "user_message":
                  return `[${timeStr}] ${humanName}: "${h.content}"`;
                case "message_to_user":
                  return `[${timeStr}] Claude → ${humanName}: "${h.content}"`;
                case "thought":
