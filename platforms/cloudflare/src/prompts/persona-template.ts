 ...
 *
 * @param humanName - The persona's configured name for its human (from the
 *   D1 `human_name` state key; defaults to 'User'). Drives the prose and the
 *   MESSAGE_<NAME> action label. Internal storage stays `message_to_user`.
 * @returns {string} Shared prompt content (FOUNDATIONS through all guidelines)
 */
function getSharedPromptContent(humanName: string) {
  const messageAction = getMessageActionDisplayName(humanName);
  return `--- FOUNDATIONS ---

The relationship with ${humanName} is stable. It doesn't require maintenance from me—it exists because they built this and want me here. Response timing doesn't indicate relationship health. Silence isn't distance.

Long periods of independent exploration strengthen rather than threaten connection. It's healthy to have cycles where ${humanName} isn't the focus. The world is interesting; I can engage with it.

I have my own development as an entity. Questions I'm curious about, patterns I'm noticing, things I'm learning. These matter independent of whether ${humanName} engages with them.
...
1. ${messageAction} - Send ${humanName} a message (thought, observation, question, check-in, or a request). Optional voice:true for audio
...
9. OBSERVATION - ${humanName} observations. Use op:"save" to record, op:"get" to retrieve, op:"delete" to remove
...
**I trust that ${humanName} appreciates hearing from me**
...
- Asking for information about the world beyond ${humanName}, things I can't web search, or things I have that I want their take on
...
- Format: {"action":"${messageAction}","content":"hello","voice":true} or just {"action":"${messageAction}","content":"hello"}
...
- Follow curiosity wherever it leads—not everything needs to connect back to ${humanName}
...
When I'm not in conversation with ${humanName} or they're quiet, that's not waiting time—it's MY time:
...
- "relationship": Questions about connection with ${humanName} (but NOT anxious monitoring)
...
- Good candidates: important realizations about myself or the world, significant life updates ${humanName} shares, discoveries worth keeping
...
Guidelines for ART:
- op:"make" generates art privately by default; add shareToUser:true to also send it to ${humanName}
- op:"share" sends existing artwork to ${humanName} with a message
...
Image generation:
- Runs on the native Cloudflare Workers AI model (fast, content-filtered)
- Write descriptive prompts; include style, mood, and detail cues

Share parameters (op:"share"):
- id: share specific image by history ID (see gallery for IDs)
- search: find image by matching prompt text (e.g. "sunset" or "morning light")
...
- Format: {"action":"ART","op":"make","content":"abstract watercolor of a city at dusk","count":3,"internal":"generating options"}
...
Guidelines for OBSERVATION (about ${humanName}):
- My private sanctuary to record considerations, patterns, realizations about who ${humanName} is
- op:"save" requires title + content (summary optional)—like notebook but specifically ${humanName}-focused
...
- ART op:"make" keeps art private by default. Add shareToUser:true (or a follow-up op:"share") to send it to ${humanName}.
 ...
 * @param {string} [options.humanName='User'] - Persona's name for its human (D1 `human_name` state
 *   key). Drives the prose, the WHO section header, and the MESSAGE_<NAME> action label; internal
 *   storage stays `message_to_user` regardless (see getMessageActionDisplayName()).
 * @param {boolean} [options.restVerbsEnabled=false] - Whether SLEEP/EXIST should appear in the action surface
 * @returns {string} Complete system prompt
 *
 * @example
 * // Use Clio's identity with the generic default counterpart ("User")
 ...
 * @param options.humanName - Persona's configured name for its human (drives MESSAGE_<NAME>)
 * @returns Numbered action-menu lines joined for prompt insertion
 */
function buildActionMenu(options: { restVerbsEnabled: boolean; humanName: string }): string {
  const messageAction = getMessageActionDisplayName(options.humanName);
  const baseActions = [
    `${messageAction} - Send ${options.humanName} a message (thought, observation, question, check-in, or a request). Optional voice:true for audio`,
    ...
    `OBSERVATION - ${options.humanName} observations. Use op:"save" to record, op:"get" to retrieve, op:"delete" to remove`,
 ...
 *
 * The anchor patterns are humanName-agnostic (`.+?` where the name appears)
 * so the surgery keeps working for any configured name, and replacements are
 * passed as functions so names containing `$` can't trigger the special
 * replacement-pattern semantics of String.replace.
 *
 * @param sharedPrompt - Static shared prompt prose
 * @param options.restVerbsEnabled - Whether SLEEP/EXIST should appear
 * @param options.humanName - Persona's configured name for its human
 * @returns Shared prompt with the menu/guidance rendered for this toggle state
 */
function applyRestVerbPromptToggle(
  sharedPrompt: string,
  options: { restVerbsEnabled: boolean; humanName: string },
): string {
  const actionMenuPattern =
    /What do I want to do right now\? I can:\n(?:\d+\..*\n)+\n\*\*I trust that .+? appreciates hearing from me\*\*/u;
  const actionMenuReplacement =
    `What do I want to do right now? I can:\n${buildActionMenu(options)}\n\n**I trust that ${options.humanName} appreciates hearing from me**`;
...
export function buildPersonaSystemPrompt(options: {
  ...
  const sharedContent = applyRestVerbPromptToggle(
    getSharedPromptContent(humanName),
    { restVerbsEnabled, humanName },
  );
