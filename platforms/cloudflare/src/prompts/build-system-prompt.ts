...
function formatHistoryEntry(
  ...
  const humanLabel = humanName.toUpperCase();
  ...
  switch (h.type) {
    case "user_message": {
      ...
      return `[${timeStr}] ${humanLabel}: "${h.content}"${hasImage ? (collectImages ? " [sent an image - see below]" : " [sent an image]") : ""}`;
    }
    case "message_to_user":
      return `[${timeStr}] MESSAGED ${humanLabel}: "${h.content}"`;
    case "thought":
    ...
    case "user_art": {
      ...
      return `[${timeStr}] ${humanLabel} MADE ART: "${h.internal?.replace(/^User's prompt:\s*/, "") || "untitled"}"${isUserArtImage && collectImages ? " [see USER'S IMAGES section]" : ""}`;
    }
    case "art_shared":
      return `[${timeStr}] SHARED ART WITH ${humanLabel}: "${h.content}"`;
    case "user_video": {
      const hasVideoGif =
        h.internal &&
      ...
      return `[${timeStr}] ${humanLabel} SENT VIDEO: "${h.content || "video"}"${hasVideoGif && collectImages ? " [see USER'S IMAGES section]" : ""}`;
    }
    case "note_saved":
 ...
 * @param {string} humanName - Configured name for the human (D1 `human_name`, default 'User')
...
export async function buildSystemPrompt(
  ...
  // Persona's configured name for its human (README/SETUP: D1 state key
  // `human_name`; generic "User" when unset). Drives prompt prose, history
  // labels, and the MESSAGE_<NAME> action label — storage types never change.
  // (Cast: this file's `db` param is annotated D1Database but callers pass
  // the Drizzle-wrapped handle getState expects — the same pre-existing
  // mismatch every sibling getState call in this file carries.)
  const humanName =
    (
      (await getState(db as unknown as Parameters<typeof getState>[0], "human_name")) || ""
    ).trim() || "User";
...
This preserves important information while freeing up context space.\n\n`
  ...
  const historyTailText = history
    .map((h) => formatHistoryEntry(h, [], [], false, humanName))
    .join("\n");
  ...
  const formatters: Parameters<typeof buildContext>[1] = {
    ...
    formatHistory: ((
      ...
      formatHistorySection(entries, options, humanName)) as unknown as Parameters<
      typeof buildContext
    >[1]["formatHistory"],
