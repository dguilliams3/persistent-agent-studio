...
> **Provenance (2026-07-31):** drawn against the original integration, which delivered
> over Telegram/Discord. In this distribution the delivery edge is **bring-your-own**: no
> channel adapter ships, the `POST /telegram` webhook does not exist, and every
> "Send to Discord/Telegram" edge below reads as "hand to your `MessagingService`
> adapter (documented no-op until you wire one — see SETUP.md)". The topology is
> otherwise as built.
