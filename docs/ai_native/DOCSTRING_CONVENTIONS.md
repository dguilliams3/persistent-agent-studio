...
**Note:** This is an illustrative example showing how to document a database module. The actual branch management code is in `packages/db/src/branches/`.
...
| `packages/memory/src/context/blocks/` | Module-level docstrings with block contracts |
...
**Note:** Database operations like history and state live in `packages/db/src/` (with platform-specific wrappers in `platforms/cloudflare/src/db/`).
