...
**Last Updated:** 2026-01-12 (paths and cost framing reconciled 2026-07-31)
**Status:** Production (deployed 2026-01-11)

> **Provenance (2026-07-31):** written pre-extraction. The retrieval math now lives in
> `packages/memory/src/rag/` (scoring, MMR, storage) with platform glue in
> `platforms/cloudflare/src/services/`; `RAG_CONFIG` lives in
> `platforms/cloudflare/src/config/index.ts` (re-exported via `constants.ts`). The
> Telegram `/rag` commands documented below come from the original integration and do
> **not** ship (no channel adapter does — see SETUP.md); the live controls are the
> settings API (`getRagConfig()` state keys) and the web UI. Where this doc and the code
...
buildSystemPrompt(db, env)
       │
       ├─► Load history (split into prefix/tail)
       │
       ├─► Check RAG config
       │   - Enabled?
       │   - Has env.AI?
       ...
       └─► RAG Retrieval Pipeline
           │
           ├─► 1. Generate query embedding from tail text
           │      (BGE-base-en-v1.5, 768 dimensions)
           │
           ├─► 2. Fetch ALL summaries with embeddings
           │      (including archived summaries)
           │
           ├─► 3. Score each summary
           │      - Similarity (cosine, 50% weight)
           │      - Recency (exp decay, 30% weight)
           │      - Importance (log scale, 20% weight)
           │
           ├─► 4. Filter by minSimilarity threshold (default 0.3)
           │
           ├─► 5. Apply MMR for diversity
           │      λ * relevance - (1-λ) * max_sim_to_selected
           │
           ├─► 6. Select top-K summaries (default 3)
           │
           └─► 7. Format as "Relevant Past Context" section
...
**Serialization:**
```javascript
// Store: Float32Array → ArrayBuffer → BLOB
...
// Retrieve: BLOB → ArrayBuffer → Float32Array
...
**Storage Size:** 768 floats × 4 bytes = 3,072 bytes per embedding (~3KB)
...
```
final_score = (α * similarity) + (β * recency) + (γ * importance)
...
while len(Selected) < topK and len(Remaining) > 0:
    ...
    for each candidate in Remaining:
        if Selected is empty:
            ...
            mmr_score = λ * candidate.combined_score - (1-λ) * max_sim
...
λ = 1.0: Pure relevance (MMR disabled, just use combined score)
λ = 0.7: Balanced (70% relevance, 30% diversity penalty) [DEFAULT]
λ = 0.5: Equal weight to relevance and diversity
λ = 0.0: Maximum diversity (ignores relevance entirely)
```

**Default:** `λ = 0.7` provides good balance between relevant and diverse results.
...
- Candidate 4: mmr = 0.7*0.65 - 0.3*0.20 = 0.455 - 0.060 = 0.395 ⭐ BEST
...
- Candidate 5: mmr = 0.7*0.62 - 0.3*max(0.15, 0.12) = 0.434 - 0.045 = 0.389 ⭐ BEST
- Selected: [1, 4, 5]
...
Defined in `platforms/cloudflare/src/config/index.ts` (re-exported via `constants.ts`):
...
**Retrieval precedence:** State value → Default from `RAG_CONFIG` → Fallback
...
```
┌─────────────────────────────────────────────────────────────┐
│ Block 1: Static System Prompt (CACHED - long TTL)          │
│ - Action reference, identity, capabilities                  │
└─────────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────────┐
│ Block 2: Stable Context (CACHED - long TTL)                │
│ - Cold storage, notebook, observations, branch summaries    │
└─────────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────────┐
│ Block 3: History Prefix (CACHED - short TTL)               │
│ - Older history entries (stable, rarely changes)            │
└─────────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────────┐
│ Block 4: Fresh Tail (NEVER CACHED)                         │
│                                                               │
│ ## Relevant Past Context                                    │
│ [RAG RESULTS INSERT HERE]                                   │
│                                                               │
│ ## Recent Activity                                          │
│ [Fresh history tail]                                        │
│                                                               │
│ ## Current Status                                           │
│ [User status, reminders, cycle count]                       │
└─────────────────────────────────────────────────────────────┘
```
...
| **Complexity** | O(N × D) where N=summaries, D=dimensions (768) |
...
**Cost estimate (in tokens — prices rot, token counts hold):**
```
~445 extra input tokens per cycle, uncached (RAG results change every cycle)
At 144 cycles/day (10-min interval), RAG adds ~64K input tokens/day
```
...
| Component | Size |
|-----------|------|
| **Embedding (Float32Array)** | 768 × 4 bytes = 3,072 bytes |
| **Candidate List (N=50)** | 50 × 3KB = 150KB |
| **Selected Results (K=3)** | 3 × 3KB = 9KB |
...
**Location:** `platforms/cloudflare/src/prompts/build-system-prompt.ts` (data loading + RAG retrieval; the pure assembly is `buildContext()` in `packages/memory/src/context/builder/`)
**Purpose:** Construct full system prompt with RAG integration
**Upstream:** runThinkingCycle(), `/context` endpoint
**Downstream:** getRagConfig(), generateEmbedding(), retrieveRelevantMemories()
