...
> **Provenance (2026-07-31):** the code samples in this doc were written against the
> pre-extraction monolith (`platforms/cloudflare/src/index.js`, now gone). The *patterns*
> (history logging, [ID:N] tracking, error shapes) still hold, but the *locations* have
> moved: system prompt → `platforms/cloudflare/src/prompts/`, action handlers →
> `platforms/cloudflare/src/services/action-executor.ts` + `packages/tools/`, routes →
> `platforms/cloudflare/src/routes/registry.ts`, DB operations → `packages/db/` (+
> `platforms/cloudflare/src/db/`). Treat file paths below as historical unless they point
> at a file that exists; the layout map is `docs/architecture/PACKAGE_STRUCTURE.md`.
...
Location: `platforms/cloudflare/src/prompts/system.ts` → `MY_CONTEXT` or system prompt section (tool prompt blocks: `platforms/cloudflare/src/tools/`)
...
Location: `platforms/cloudflare/src/services/action-executor.ts` (+ tool definitions in `packages/tools/`)
...
```javascript
/**
 * Generate image via native Cloudflare Workers AI
 * (@cf/stabilityai/stable-diffusion-xl-base-1.0, content-filtered)
 */
...
```javascript
// packages/memory/src/summarization/formatter/format-entries.ts
const isBase64Image = h.content?.startsWith('data:image');
if (isBase64Image) {
...
### Real-World Example: LLM Provider Routing

```javascript
// Provider abstraction in action
async function callModel(modelId, messages, env) {
  // Feature detection based on model id
  const provider = modelId.startsWith('claude') ? 'anthropic' :
                   modelId.startsWith('gpt') ? 'openai' : 'deepseek';

  // Abstract away provider differences
  const client = createLLMClient(provider, env);

  return await client.complete(modelId, messages);
}
```
...
**Location:** anywhere a large action-description switch appears (historically the monolith; today `platforms/cloudflare/src/services/`)
