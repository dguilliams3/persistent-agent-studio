...
A structured checklist for adding or modifying features. Use this to ensure nothing is missed.

> **Provenance (2026-07-31):** written pre-extraction. Translate the file names as you
> read: the monolith (`index.js`) is gone — system prompt lives in
> `platforms/cloudflare/src/prompts/`, action handlers in
> `platforms/cloudflare/src/services/action-executor.ts` + `packages/tools/`, routes in
> `platforms/cloudflare/src/routes/registry.ts`, DB modules in `packages/db/`; the
> `CLAUDE.md` this doc says to update was the original tree's agent guide — in this repo,
> update `README.md` / `docs/` instead. Telegram commands do not ship (no channel
> adapter does). The *steps* still hold; the *paths* moved.
...
- [ ] Add action to numbered list in `getStaticSystemPrompt()` (`platforms/cloudflare/src/prompts/system.ts`)
...
- [ ] Handle loading states: `setIsLoading(true)` → try/catch → `setIsLoading(false)`
