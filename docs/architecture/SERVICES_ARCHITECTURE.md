...
┌─────────────────────────────────────────────────────────────────────────────┐
│                           EXTERNAL APIS                                      │
│  ElevenLabs    Cloudflare AI    Telegram    Discord    Brave                │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
// In platform layer (platforms/cloudflare/src/services/)
// NOTE: no concrete channel adapter ships — `@persistence/services/messaging`
// exports the MessagingService CONTRACT only, and the worker's delivery call
// sites are documented no-ops until you wire your own adapter (see SETUP.md).

import type { MessagingService } from '@persistence/services/messaging';

const messaging: MessagingService = new YourChannelAdapter(env.YOUR_CHANNEL_TOKEN);
