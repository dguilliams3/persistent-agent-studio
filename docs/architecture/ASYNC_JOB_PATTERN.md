...
- **Slow external prediction APIs** - 10-60 seconds
...
export interface PendingJob {
  ...
  service: string;               // 'batch', 'myservice', etc.
...
## Example: Polling an External Prediction API
...
export class PredictionService implements AsyncJobService<PredictionInput, PredictionResult> {
  constructor(private token: string) {}

  async startJob(input: PredictionInput): Promise<{ jobId: string }> {
    const response = await fetch('https://api.example.com/v1/predictions', {
      method: 'POST',
      headers: {
  ...
  async checkJob(jobId: string): Promise<AsyncJob<PredictionResult>> {
    const response = await fetch(
      `https://api.example.com/v1/predictions/${jobId}`,
  ...
  private mapStatus(remoteStatus: string): JobStatus {
    switch (remoteStatus) {
      case 'starting':
      case 'processing':
        return 'processing';
...
export async function processPendingJobs(
  db: D1Database,
  services: ServiceRegistry,  // { myservice: PredictionService, ... }
...
export const handler: ToolHandler<ArtParams> = async (params, ctx) => {
  ...
  const predictions = new PredictionService(env.PREDICTION_API_TOKEN);
  const { jobId } = await predictions.startJob({
...
CREATE TABLE IF NOT EXISTS pending_jobs (
  ...
  service TEXT NOT NULL,          -- 'batch', 'myservice'
