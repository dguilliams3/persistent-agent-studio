...
> **Provenance (2026-07-31):** this is a dated **price snapshot** — hard dollar figures
> rot, which is why the repo's cost narrative elsewhere (README, docs, the web app's
> Efficiencies page) is stated in **ratios** (cache read ≈ 0.1x, write premium 1.25x/2x,
> batch ≈ 0.5x). The prices the code actually uses live in `MODEL_PRICING` /
> `CACHE_PRICING` (`packages/core/src/constants.ts`); verify against Anthropic's pricing
> page before trusting any absolute number below.
