# Persistent Agent Studio - Entity Relationship Diagram (ERD)

**Last Updated:** 2026-01-21 (code-usage paths reconciled 2026-07-31)
**Database:** Cloudflare D1 (SQLite-compatible)
**Current Schema Version:** v25 (Summary Tier Refactor)

> **Provenance (2026-07-31):** written pre-extraction under the project's original name
> ("Claude Existence Loop"), against a `worker/src/` layout that is now
> `platforms/cloudflare/src/` (+ `packages/db/`). The table shapes are still accurate to
> the shipped migrations (`platforms/cloudflare/migration_v*.sql` is the ground truth);
> the "Code Usage" pointers have been updated to the extracted layout.
...
Each persona maintains isolated memories while sharing infrastructure tables. See `platforms/cloudflare/src/db/fork.ts` for smart-copy persona forking implementation.
...
**Code Usage:**
- `packages/db/src/personas.ts` - All persona operations
- `platforms/cloudflare/src/db/fork.ts` - Persona forking (smart copy strategy)
...
**Code Usage:**
- `platforms/cloudflare/src/utils/history-logger.ts:logHistory()` - **Preferred** - Auto-captures meter snapshot
- `packages/db/src/history.ts` - Low-level CRUD operations
- `getHistoryForContext()` (`platforms/cloudflare/src/db/`) - Context building
...
**Code Usage:**
- `packages/db/src/cycles.ts` - `createCycle()`, `updateCycleMetrics()`, `markCycleError()`
- `runThinkingCycle()` (`packages/runtime/src/orchestrator/`) - Creates and updates cycles
...
**Code Usage:**
- `packages/db/src/state.ts` - `getState()`, `setState()`
...
**Code Usage:**
- `packages/db/src/llm-storage/` - `getColdStorage()`, `addColdStorage()`
...
**Code Usage:**
- `packages/db/src/llm-storage/` - `getNotebook()`, `getNote()`, `saveNote()`, `deleteNote()`
...
**Code Usage:**
- `packages/db/src/summaries/` - All summary operations
...
**Code Usage:**
- `packages/db/src/llm-storage/reminders.ts` - CRUD operations
...
**Code Usage:**
- `packages/db/src/llm-storage/` - CRUD operations
...
**Code Usage:**
- `packages/db/src/llm-storage/learned.ts` - All operations
...
**Code Usage:**
- `packages/db/src/llm-storage/questions.ts` - All operations
...
**Code Usage:**
- `packages/db/src/branches/` - Branch management
- `platforms/cloudflare/src/routes/branches.ts` - API endpoints
...
**Code Usage:**
- `packages/db/src/branches/` - Override operations
- `platforms/cloudflare/src/routes/branches.ts` - `/memory/exclude`, `/memory/edit` endpoints
...
**Code Usage:**
- `packages/db/src/branches/` - Synthetic memory CRUD
...
**Code Usage:**
- `packages/db/src/pinned.ts` - `pinImage()`, `unpinImage()`, `swapPinnedImages()`
...
**Code Usage:**
- `packages/db/src/pinned.ts` - `requestViewImages()`, `clearViewedImages()`
...
**Code Usage:**
- `platforms/cloudflare/src/db/batches.ts` - Batch operations
...
**Code Usage:**
- `platforms/cloudflare/src/routes/registry.ts` - `/tts-generate`, `/voice-history` endpoints
- `platforms/cloudflare/src/voice/` - Voice playback/transcription support
...
**Code Usage:**
- `platforms/cloudflare/src/db/glossary.ts` - CRUD operations, applyGlossary(), buildGlossaryPrompt()
- `platforms/cloudflare/src/routes/glossary.ts` - `/glossary` API endpoints
  *(the original integration's `/glossary` Telegram command does not ship — no channel adapter does)*
...
| v18-v22 | (various internal) | See `platforms/cloudflare/src/routes/migrate.ts` for migration handlers |
