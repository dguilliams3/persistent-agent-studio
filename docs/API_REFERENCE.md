...
> Comprehensive reference for the worker's REST API.

**Base URL:** `https://your-worker.workers.dev`

**Last Updated:** 2026-02-03

> **Provenance (2026-07-31):** written against the original integration, which included a
> Telegram bot. This distribution ships **no channel adapter**, so the Telegram surfaces
> documented below (webhook endpoints `POST /telegram`, `POST /test-telegram`,
> `POST /reset-telegram-webhook`, the Telegram-initData auth path, and the "Telegram Bot
> Commands" section) do **not** exist in the deployed worker — they are retained as
> history only. The authoritative route inventory is
> `platforms/cloudflare/src/routes/registry.ts`; when this doc and the registry disagree,
> the registry wins.
