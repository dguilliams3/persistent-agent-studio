...
├── elevenlabs/                  ├── tts/
├── cloudflare-ai/               │   ├── types.ts (interface)
├── telegram/                    │   ├── elevenlabs.ts
└── discord/                     │   └── google.ts
                                 ├── image_generation/
                                 │   ├── types.ts
                                 │   └── cloudflare.ts
...
> **Note (2026-07-31):** the `runs/RUN-*` documents cited below were working artifacts of
> the original private development tree; they were not carried into this repository. The
> decisions stand on their own — the citations mark provenance, not readable links.
...
**Decision:** Organize services by capability (tts/, stt/) not provider (elevenlabs/, etc.)
