...
> **Provenance (2026-07-31):** the "code migration in progress" caveats below are stale —
> persona scoping shipped (see `packages/db/src/persona-scope.ts` and its tests), and the
> `tech_debt.md` file this doc references was not carried into this repository. The
> "Telegram Commands (Planned)" section describes the original integration's roadmap; no
> channel adapter ships in this distribution (see SETUP.md).
