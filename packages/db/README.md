...
**Pricing included from @persistence/core:** per-model rates live in `MODEL_PRICING`
(`packages/core/src/constants.ts`) — read them there rather than from a doc snapshot
(prices rot; the constants are what `calculateCostCents()` actually uses). The cache
ratios (`CACHE_PRICING`): reads ≈ 0.1× the input rate, write premium 1.25× (5-min TTL) /
2× (1-hour TTL), batch ≈ 0.5×.
