...
describe('NodeEnvSecretsProvider', () => {
  ...
  describe('real-world scenarios', () => {
    ...
    it('handles optional feature toggles', async () => {
      process.env.FEATURE_ALPHA = 'true';
      // FEATURE_BETA not set

      const alphaEnabled = await provider.has('FEATURE_ALPHA');
      const betaEnabled = await provider.has('FEATURE_BETA');
