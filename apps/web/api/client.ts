...
/**
 * Demo-mode stand-in password. The exhibit's API discards password values
 * entirely, but shared plumbing (store actions, the demo personality import)
 * refuses an empty one — a named constant satisfies the contract without
 * asking visitors for a secret that doesn't exist (same rationale as the
 * SyntheticMemory delete strip, ledger §2 row 17).
 */
const DEMO_ADMIN_PASSWORD = 'demo';

/**
 * Resolve the admin password without any native dialog: env override →
 * sessionStorage → null. This used to fall through to window.prompt(), the
 * last native-dialog path in the app (ledger §2 row 17 residue) — callers
 * that get null now collect the password through their own in-UI field and
 * persist it via setAdminPassword().
 */
...
/**
 * Remember an admin password collected by an in-UI field (OverridesPanel,
 * PersonalityPanel) for the rest of the session — the same persistence the
 * old prompt() path provided.
 */
