...
interface PersonalityPanelProps {
  ...
  // `password` is required by the store action in 'replace' mode and omitted
  // otherwise — mirrors EditorSlice.importPersonality in store/slices/editor.ts.
  importPersonality: (
...
export function PersonalityPanel({
  ...
  // Replace-mode admin password, collected in-UI (no window.prompt —
  // ledger §2 row 17 residue). Blank falls back to env/session storage.
  ...
  const handleImport = useCallback(async () => {
    ...
    const entered = replacePassword.trim();
    const password = importMode === 'replace' ? entered || getAdminPassword() : undefined;
    if (importMode === 'replace' && !password) {
      addLog('❌ Admin password required for replace mode — enter it in the field above.');
      return;
    }
  ...
  }, [addLog, importMode, importPersonality, readImportFile, replacePassword, validateSnapshot]);
          ...
          {importMode === 'replace' && (
            <label style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-xs)' }}>
              <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>
                Admin password (required for replace)
              </span>
              <input
                type="password"
                value={replacePassword}
                onChange={(event) => setReplacePassword(event.target.value)}
                placeholder="Leave blank to use the saved session password"
                aria-label="Admin password for replace import"
                style={fieldStyle}
              />
            </label>
          )}
