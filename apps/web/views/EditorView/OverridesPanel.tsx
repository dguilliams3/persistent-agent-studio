...
export function OverridesPanel({ activeBranch }: { activeBranch: string }) {
  ...
  // In-UI admin password collection (no window.prompt — ledger §2 row 17
  // residue): when no password is stored, the row grows an inline field.
  ...
  const performUndo = useCallback(
    async (overrideId: number, password: string) => {
  ...
  const handleUndo = useCallback(
    async (overrideId: number) => {
      ...
      if (!password) {
        // No env/session password: open the row's inline field instead of a
        // native prompt.
        setPasswordForId(overrideId);
        setPasswordValue('');
        return;
      }
      await performUndo(overrideId, password);
    },
    [performUndo],
  );

  const confirmPasswordUndo = useCallback(async () => {
    const entered = passwordValue.trim();
    if (passwordForId === null || !entered) {
      return;
    }
    setAdminPassword(entered);
    setPasswordForId(null);
    setPasswordValue('');
    await performUndo(passwordForId, entered);
  }, [passwordForId, passwordValue, performUndo]);

  const cancelPasswordUndo = useCallback(() => {
    setPasswordForId(null);
    setPasswordValue('');
  }, []);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)' }}>
          {overrides.map((override) => (
            ...
            >
              ...
              {/* Inline admin-password confirmation — replaces window.prompt */}
              {passwordForId === override.id && (
                <div
                  style={{
                    display: 'flex',
                    flexWrap: 'wrap',
                ...
                >
                  <input
                    ...
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') void confirmPasswordUndo();
                      if (event.key === 'Escape') cancelPasswordUndo();
                    }}
                    placeholder="Admin password"
                    aria-label={`Admin password to undo override ${override.id}`}
                    autoFocus
                    style={{
                      flex: 1,
                      minWidth: '160px',
                  ...
                  <button
                    type="button"
                    onClick={() => void confirmPasswordUndo()}
                    disabled={!passwordValue.trim()}
                    aria-label={`Confirm undo of override ${override.id}`}
                    style={{
                      minHeight: '36px',
                      padding: '0 var(--spacing-md)',
                      borderRadius: 'var(--radius-sm)',
                      ...
                      backgroundColor: 'var(--accent)',
                      color: 'var(--text-primary)',
                      cursor: passwordValue.trim() ? 'pointer' : 'not-allowed',
                      opacity: passwordValue.trim() ? 1 : 0.5,
                    }}
                  >
                    Undo
                  </button>
