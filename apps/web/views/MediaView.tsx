...
import { dayLabel } from '../components/chat/segmentHistory';
...
/**
 * Group entries by day, labeled with the chat surface's canonical day
 * convention (Today / Yesterday / "Jun 20"). dayLabel parses D1's bare-UTC
 * timestamps correctly — the raw `new Date(...).toLocaleDateString` this
 * replaces shifted every stamp by the viewer's UTC offset, the same
 * timezone-skew class chat fixed long ago (ledger §2 row 13).
 */
function groupByDay(items: any[]): Map<string, any[]> {
  const groups = new Map<string, any[]>();
  for (const item of items) {
    const date = (item.created_at && dayLabel(item.created_at)) || 'Unknown';
