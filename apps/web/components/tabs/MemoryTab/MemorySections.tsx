 ...
 *
 * Delete confirms in-card (no native confirm()/prompt() — ledger §2 row 17
 * pattern): the demo exhibit shows a plain confirm strip, a real deployment
 * adds the admin-password field the worker requires.
 *
 * @upstream Called by: MemoryTab
...
export function LearningsSection({
  ...
  // One confirm strip at a time; password only in real deployments.
  ...
  const confirmDelete = (id: string) => {
    ...
    // The store refuses an empty password; the demo API discards the value,
    // so a named constant satisfies the shared plumbing without asking
    // visitors for a secret that doesn't exist.
    void onDelete(id, DEMO_MODE ? "demo" : deletePassword.trim());
    cancelDelete();
  };
        ...
        {learned.map((item) => (
          ...
          >
            ...
            <div className="px-3 pb-3 space-y-3">
              ...
              {/* In-card delete confirmation — replaces confirm() + prompt() */}
              {confirmingId === item.id && (
                <div className="flex flex-wrap items-center gap-2 pt-2">
                  <span className="text-xs text-content-secondary flex-1 min-w-[160px]">
                    Delete learning #{item.id}? This can't be undone.
                  </span>
                  {!DEMO_MODE && (
                    <input
                      type="password"
                      ...
                      onKeyDown={(event) => {
                        if (event.key === "Enter") confirmDelete(item.id);
                        if (event.key === "Escape") cancelDelete();
                      }}
                      placeholder="Admin password"
                      aria-label={`Admin password to delete learning ${item.id}`}
                      autoFocus
                      className="px-2 py-1 text-xs rounded-md bg-depth border border-border-subtle text-content-primary min-h-[32px]"
                    />
                  )}
                  <button
                    onClick={(e) => {
                    ...
                    className="text-xs px-2 py-1 rounded border border-border-subtle text-content-secondary hover:bg-depth transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="text-xs px-2 py-1 rounded bg-danger/90 hover:bg-danger text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Delete
                  </button>
                </div>
