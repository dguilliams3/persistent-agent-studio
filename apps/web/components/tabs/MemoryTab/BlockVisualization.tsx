 ...
 *
 * Cache Architecture (as built — packages/memory/src/context/blocks/):
 * - Block 1 (blue): Static constitution + extensions (never changes, cached 1hr)
 * - Block 2 (green): Promoted summaries — pinned summaries that bypass rotation
 * - Block 3 (yellow): Stable context — observations about the user + older summaries prefix
 * - Block 4 (red): Fresh tail — learned, questions, notebook, RAG recall, summary
 *   tail, full history, reminders, status, meters (never cached, always input)
 *
 * Blocks 1-3 save 90% of token cost when hit in cache. Block 4 is always uncached.
 *
 * NOTE: the /context payload keys are legacy names — `block2_stable` carries
 * block2_promotedSummaries tokens and `block3_summariesPrefix` carries
 * block3_stableAndSummaries tokens (see the /context route in
 * platforms/cloudflare/src/routes/registry.ts). An earlier revision of this
 * file read the key names literally and described the superseded layout
 * (learned/questions/notebook in Block 2); those sections live in Block 4 now
 * so they are never stale-cached.
 *
 ...
 * @param {number} props.tokenBreakdown.block2_stable - Promoted-summaries tokens (legacy key name; cached)
 * @param {number} props.tokenBreakdown.block3_summariesPrefix - Stable context + summaries-prefix tokens (cached)
 ...
 * @param {number} props.tokenBreakdown.block2_stable - Promoted-summaries tokens (legacy key name)
 * @param {number} props.tokenBreakdown.block3_summariesPrefix - Stable context + summaries-prefix tokens
 * @param {number} props.tokenBreakdown.block4_fresh - Fresh tail tokens (uncached)
...
export default function BlockVisualization({ tokenBreakdown }: BlockVisualizationProps) {
  ...
  const blocks = [
    ...
    },
    {
      id: 'block2',
      name: 'Block 2',
      label: 'Promoted Summaries',
      tokens: block2_stable,
      color: 'bg-green-500',
      cached: true,
      description: 'Pinned summaries that bypass rotation (cached)'
    },
    {
      id: 'block3',
      name: 'Block 3',
      label: 'Stable + Summaries',
      tokens: block3_summariesPrefix,
      color: 'bg-yellow-500',
      cached: true,
      description: 'Observations about the user + older summaries prefix (cached)'
    },
    {
      id: 'block4',
      name: 'Block 4',
      ...
      color: 'bg-red-500',
      cached: false,
      description: 'Learned, questions, notebook, RAG recall, latest history (never cached, always input)'
    }
  ];
