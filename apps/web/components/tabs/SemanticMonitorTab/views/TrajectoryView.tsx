 ...
 *
 * This view renders a scatter/line chart showing how the persona's entries drift relative
 * to its established semantic identity. Each point represents an embedded entry, with:
...
export function TrajectoryView({ autoFetch = false }: TrajectoryViewProps) {
  ...
  return (
    <div className="space-y-3 h-full">
      {/* Header row. Both levels wrap: at 1024–1439px the docked tool panel
          is still ~340px (the ≥1440px clamp in SplitView doesn't apply), and
          the filter + limit + Load controls exceed that — without wrap the
          Load button clipped to "↻ Loa" (ledger §2 row 9 residue). */}
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end min-w-0">
          ...
          >
            {LIMIT_OPTIONS.map((opt) => (
              <option key={opt.value} value={opt.value} style={{ backgroundColor: 'var(--surface-raised)' }}>
                {opt.label}
              </option>
            ))}
          </select>
