 ...
 *   - usePersonaName - active persona's display name for type labels
...
/**
 * @description Memory type options with icons and descriptions.
 *
 * The `value`s are the storage entry types the worker persists — never
 * touched. Only the labels are display, and the art label names whichever
 * persona lives here rather than the reference one.
 *
 * @param {string} personaName - Active persona's display name
 * @returns {Array<{value: string, label: string, description: string}>} Options
 */
function buildTypeOptions(personaName: string) {
  return [
    ...
    { value: 'art_result', label: `🖼️ ${personaName}'s Art`, description: 'Generated artwork' },
...
export default function SyntheticMemory() {
  ...
  const fetchSyntheticMemories = useAppStore((s) => s.fetchSyntheticMemories);

  /**
   * Fetch on mount: fetchAll only refreshes synthetics after the Editor tab
   * has been visited, so opening SIM → Memory Lab directly showed "No
   * synthetic memories yet" while the branch chip counted them (+2). Same
   * mount-fetch pattern as MediaView.
   */
  useEffect(() => {
    void fetchSyntheticMemories();
  }, [fetchSyntheticMemories, activeBranch]);
...
function SyntheticForm({ syntheticType,
  ...
  const typeOptions = useMemo(() => buildTypeOptions(personaName), [personaName]);
 ...
 *
 * Delete confirms in-card (no window.prompt — Gemini P1-UX, ledger §2 row
 * 17): the demo exhibit shows a plain confirm strip, a real deployment adds
 * the admin-password field the worker requires.
 */
function SyntheticCard({ synth, openSyntheticEdit, deleteSyntheticMemory }: any) {
  ...
  const canConfirmDelete = DEMO_MODE || deletePassword.trim().length > 0;

  const confirmDelete = () => {
    if (!canConfirmDelete) return;
    // The store refuses an empty password, but the demo API has no password
    // check at all and discards the value — a constant satisfies the shared
    // plumbing without asking visitors for a secret that doesn't exist.
    deleteSyntheticMemory(synth.id, DEMO_MODE ? 'demo' : deletePassword.trim());
    ...
    <div className="card p-4 hover:border-accent/30 transition-colors">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 text-xs text-content-muted mb-2">
            ...
            <span className="text-content-muted">
              {/* Chat's formatTime is the app-wide timestamp convention
                  (ledger §2 row 13) — includes the UTC-parse fix. */}
              {synth.created_at && formatTime(synth.created_at)}
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={() => setConfirmingDelete((previous) => !previous)}
      ...
      {/* In-card delete confirmation — replaces the old window.prompt() */}
      {confirmingDelete && (
        <div className="mt-3 pt-3 border-t border-border-subtle flex flex-wrap items-center gap-2">
          <span className="text-sm text-content-secondary flex-1 min-w-[180px]">
            Delete this synthetic memory? This can&apos;t be undone.
          </span>
          {!DEMO_MODE && (
            <input
              type="password"
              ...
              aria-label="Admin password to confirm delete"
              autoFocus
              className="px-3 py-1.5 text-xs rounded-md bg-depth border border-border-subtle text-content-primary min-h-[36px]"
            />
          )}
          <button
          ...
          <button
            ...
            className="btn-danger px-3 py-1.5 text-xs flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Icon name="Trash2" size={12} />
            Delete
          </button>
