import { formatTime, parseDbTimestamp } from '../../../ui/historyUtils';
...
export default function BasinMetricsCard({ basinData, loading, error, onRecompute }: BasinMetricsCardProps & { onRecompute?: () => void }) {
  ...
  const isStale = computedAt && (Date.now() - parseDbTimestamp(computedAt).getTime() > 7 * 24 * 60 * 60 * 1000);
  // Chat's formatTime is the app-wide timestamp convention (ledger §2 row
  // 13) — the raw toLocaleDateString() here printed "computed 7/30/2026"
  // beside the QuickStats tile's "1d ago" for the same stamp.
  const stalenessText = computedAt
    ? `computed ${formatTime(computedAt)} ${isStale ? '(STALE >7d)' : ''}`
    : '';
