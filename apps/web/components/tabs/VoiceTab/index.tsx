 ...
 *
 * DEMO_MODE (the exhibit) renders an honest one-panel statement instead of
 * the controls: voice needs a live deployment's ElevenLabs/realtime keys, so
 * the exhibit says that plainly rather than staging a Generate button that
 * 404s (api/demo demoFetchRaw) and config that saves nowhere.
 *
 * Allows users to:
...
export default function VoiceTab({ isPanel = false }: { isPanel?: boolean }) {
  ...
  // Fetch voice history, TTS model, and credits on mount. Skipped in the
  // exhibit: none of the voice endpoints have fixtures, and fetching what
  // nothing serves would only fill the dev console with unfixtured-endpoint
  // warnings for a surface the demo honestly declines to stage.
  useEffect(() => {
    if (DEMO_MODE) return;
    fetchTabData('voice');
  ...
  /** @description Save TTS model settings for the active persona */
  ...
  // Exhibit build: voice runs on a live deployment's ElevenLabs / realtime
  // keys, and there is nothing honest a scripted demo could play here. So no
  // dead Generate button, no config panel saving nowhere — one plain
  // statement of what this surface is when the system is actually deployed
  // (same honesty register as the chat's pending copy: describe what is
  // happening, never imply a control that isn't wired).
  if (DEMO_MODE) {
    return (
      <div className={isPanel ? 'space-y-3' : 'space-y-4'}>
        <div className={isPanel ? 'flex flex-col items-start gap-1' : 'flex items-center gap-3'}>
          ...
          <h2 className="text-display text-xl font-semibold text-content-primary">Voice</h2>
        </div>
        <div className="p-4 bg-surface rounded-lg border border-border-subtle space-y-2">
          <p className="text-sm text-content-secondary">
            Voice runs against a live deployment&apos;s TTS and realtime keys — not part
            of the exhibit. Nothing is generated here, so nothing pretends to be.
          </p>
          <p className="text-sm text-content-muted">
            Deployed, this tab speaks the persona&apos;s messages aloud (ElevenLabs TTS
            with per-persona voice config), holds realtime voice sessions, keeps a
            replayable history of every generation, and learns speech-to-text
            corrections from a glossary you curate.
          </p>
        </div>
      </div>
    );
