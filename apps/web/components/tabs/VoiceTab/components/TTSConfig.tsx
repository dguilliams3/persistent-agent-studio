 ...
 *
 * The save affordance names the persona whose voice is being set, read from
 * the active persona record — never a literal.
 *
 * @upstream Called by:
 *   - VoiceTab/index.jsx - Renders in voice testing interface
 * @downstream Calls:
 *   - Select component (from ui/)
 *   - usePersonaName() hook - active persona's display name
 */
...
export default function TTSConfig({ model,
  ...
  return (
    ...
    >
      ...
      {/* Save-for-the-persona button | Phase 7b: More prominent styling */}
      <div className="flex items-center gap-3 pt-2 border-t border-border-subtle">
        ...
        >
          {saving ? 'Saving...' : `✨ Set for ${personaName}`}
        </button>
        {saveSuccess && (
          <span className="text-sm text-success">
            {personaName} uses: {model.toUpperCase()}
            {model === 'v3'
        ...
        {!saveSuccess && (
          <span className="text-xs text-content-muted">
            Persists model for {personaName}&apos;s voice messages
          </span>
        )}
      </div>
      </div>
