...
/**
 * @description The three kinds of memory a synthetic can be, from the thread's
 * point of view. `value`s are storage entry types (never touched); the middle
 * label names whichever persona lives here — the reference persona's name and
 * its pronoun both used to be baked in.
 *
 * @param {string} personaName - Active persona's display name
 * @returns {Array<{value: string, label: string}>} Type chips
 */
function buildSyntheticTypes(personaName: string) {
  return [
    { value: 'user_message', label: 'from you' },
    { value: 'message_to_user', label: `from ${personaName}` },
...
export function InsertMemoryPoint({ timestamp, onInserted }: InsertMemoryPointProps) {
  ...
  const syntheticTypes = useMemo(() => buildSyntheticTypes(personaName), [personaName]);
