 ...
 *
 * The 'thinking' state is the chat's one pending affordance, shared by a
 * running think cycle AND a message awaiting its reply. The caller supplies
 * both the visible `statusText` and the spoken `pendingLabel`, because those
 * two waits are not the same claim and must not read as if they were.
 *
...
export interface ThinkTriggerProps {
  ...
  statusText?: string;
  /**
   * Accessible name for the dots. The pending state is reused for two
   * different claims — a cycle is thinking, or a reply is on its way — and
   * the demo exhibit's reply is a fixture, not deliberation. Announcing
   * "Thinking..." there would be the same lie in the a11y layer that the
   * visible copy is careful to avoid, so callers name what is actually
   * happening.
   */
  pendingLabel?: string;
}
...
/**
 * Three breathing dots shown while something is pending.
 * Animation: 2s ease-in-out, staggered 0s / 0.3s / 0.6s.
 * prefers-reduced-motion: static dots with no animation.
 */
function BreathingDots({ label = 'Thinking...' }: { label?: string }) {
  ...
  if (state === 'thinking') {
    return (
      <>
        ...
        <BreathingDots label={pendingLabel} />
        {statusText && (
          <div
            style={{
              textAlign: 'center',
