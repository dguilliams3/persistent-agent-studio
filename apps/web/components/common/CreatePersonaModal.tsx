...
/**
 * System-prompt templates offered at mint time.
 *
 * The `id`s are server-recognized data values (packages/db handlers/personas.ts
 * SYSTEM_PROMPT_TEMPLATES) and must not be renamed here — `clio-v1` is the
 * reference persona's original constitution, stored under that key. The labels
 * are display, and name the template's role rather than the persona that
 * happened to be built on it.
 */
const TEMPLATES: Array<{ id: string; label: string; blurb: string }> = [
  ...
  { id: 'blank', label: 'Blank', blurb: 'No starting identity at all. Pure observation.' },
  {
    id: 'clio-v1',
    label: 'Reference v1',
    blurb: "The reference persona's original constitution — continuity-focused.",
  },
];
