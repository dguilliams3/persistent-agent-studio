 ...
 * - Support text and image types (art_result)
 ...
 * When provided, branchName selects the edit branch to activate before write.
   ...
   *
   * @note Art types (art_result) with base64 content are stored as image object in state
   */
  openSyntheticEdit: (synthetic: SyntheticMemory) => {
    ...
    const isArtType = memType === "art_result";
   ...
   * For art types (art_result), uses syntheticImage.base64 as content.
  ...
  saveSyntheticMemory: async () => {
    ...
    const isArtType = syntheticType === "art_result";
