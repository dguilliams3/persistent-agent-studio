...
/** One weekly drift bucket from /sim/basin/weekly (computeWeeklyBasinBuckets) */
export interface SIMWeeklyBucket {
  week: string;
  n: number;
...
export interface SIMSlice {
  ...
  simWeeklyBuckets: SIMWeeklyBucket[];
  simWeeklyType: string;
  simWeeklyLoading: boolean;
  ...
  fetchWeeklyBasin: (type?: string) => Promise<void>;
...
export const createSIMSlice: StateCreator<AppState, [], [], SIMSlice> = (
  ...
  /** @type {SIMWeeklyBucket[]} Weekly drift buckets from /sim/basin/weekly */
  simWeeklyBuckets: [] as SIMWeeklyBucket[],

  /** @type {string} Voice type the weekly buckets were computed for */
  simWeeklyType: 'thought',

  /** @type {boolean} Loading flag for the weekly fetch */
  simWeeklyLoading: false,
  ...
  /**
   * Fetches weekly drift buckets for one voice from /sim/basin/weekly.
   * A deployment without enough embedded history answers {success:false};
   * that is an empty state, not an error, so it never trips simError —
   * the Overview panel renders its own quiet placeholder instead.
   * @param {string} [type='thought'] - Voice type (thought | message_to_user | user_message)
   * @returns {Promise<void>}
   */
  fetchWeeklyBasin: async (type: string = 'thought') => {
    set({ simWeeklyLoading: true });
    try {
      const data = await api.get(
        `/sim/basin/weekly?type=${encodeURIComponent(type)}`,
      );
      set({
        simWeeklyBuckets: Array.isArray(data.weekly)
          ? (data.weekly as SIMWeeklyBucket[])
          : [],
        simWeeklyType: type,
        simWeeklyLoading: false,
      });
    } catch {
      set({ simWeeklyBuckets: [], simWeeklyType: type, simWeeklyLoading: false });
    }
  },
  ...
  exportSIMData: async (options: Record<string, unknown> = {}) => {
    ...
    try {
      ...
      if (format === "csv") {
        // Convert trajectory array to CSV — /sim/export's trajectory rows are
        // flat point records (handleSimExport), minus the embedding column.
        const rows = Array.isArray(data.trajectory)
          ? (data.trajectory as Array<Record<string, unknown>>)
          : [];
        if (rows.length === 0) {
