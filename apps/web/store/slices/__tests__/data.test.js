...
vi.mock('../../../api/client', () => ({
  ...
  // Tests exercise the real-deployment paths; the exhibit's voice-prefetch
...
describe('Data Fetching Slice', () => {
  ...
  describe('deleteLearning', () => {
    // The password now arrives from the UI's in-card confirm strip
    // (LearningsSection) — the store never opens a native prompt().
    ...
    it('deletes learning with password', async () => {
      ...
      const { getState } = store;
      const result = await getState().deleteLearning(123, 'admin-password');
    ...
    it('refuses empty password', async () => {
    ...
    it('handles delete errors', async () => {
      ...
      const { getState } = store;
      const result = await getState().deleteLearning(123, 'wrong-password');
  ...
  describe('fetchTabData', () => {
    ...
    it('voice tab loads voice tooling data', async () => {
      ...
      expect(mockFetches.addLog).toHaveBeenCalledWith('🎤 Loading voice tools...');
    });
