...
export interface DataSlice {
  ...
  deleteLearning: (id: string, password: string) => Promise<boolean>;
...
export const createDataSlice: StateCreator<AppState, [], [], DataSlice> = (
   ...
   *
   * Retrieves the persona's observations of the user's behavior and preferences.
   *
   * @upstream Called by: fetchAll, memory tab initialization
   ...
   *
   * Retrieves the persona's verified knowledge about itself.
   *
   * @upstream Called by: fetchAll, memory tab initialization
   ...
   *
   * Removes a learned entry. The admin password comes from the caller's
   * in-UI confirm strip (LearningsSection) — the old window.prompt() path
   * was the last native dialog in this slice (ledger §2 row 17 residue).
   *
   * @upstream Called by: LearningsSection in-card delete confirmation
   * @downstream Calls: api.delete('/learned/:id'), addLog, fetchLearned
   *
   * @param {string} id - Learned entry ID
   * @param {string} password - Admin password collected by the UI
   * @returns {Promise<boolean>} True if delete succeeded, false otherwise
   *
   * @tests apps/web/store/slices/__tests__/data.test.js
   *   - "deleteLearning - success with password"
   *   - "deleteLearning - refuses empty password"
   *   - "deleteLearning - error handling"
   */
  deleteLearning: async (id: string, password: string) => {
   ...
   *
   * Retrieves questions the persona is currently holding or exploring.
   *
   * @upstream Called by: fetchAll, memory tab initialization
  ...
  fetchAll: async () => {
    ...
    try {
      ...
      // Voice is honestly disabled in the exhibit (no fixtures serve these
      // endpoints); fetching would only warn "[demo] no fixture" per call.
      if (visitedTabs.has("voice") && !DEMO_MODE) {
        coreFetches.push(
          fetchVoiceHistory(),
          fetchTTSModel(),
          fetchTTSCredits(),
  ...
  fetchTabData: async (tab: string) => {
    ...
    try {
      switch (tab) {
        ...
        case "voice":
          // The exhibit declines to stage voice: none of these five endpoints
          // have fixtures, so in demo mode this prefetch would only fill the
          // console with unfixtured-endpoint warnings.
          if (DEMO_MODE) break;
          addLog("🎤 Loading voice tools...");
          await Promise.all([
            fetchVoiceHistory(),
            fetchTTSModel(),
            fetchTTSCredits(),
