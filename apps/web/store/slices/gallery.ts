...
export interface GallerySlice {
  ...
  setSaveAsPersonaArt: (asPersonaArt: boolean) => void;
...
> = (set, get) => ({
  ...
  /** @type {boolean} Save generated image as the persona's art (true) or the user's art (false) */
  saveAsPersonaArt: true,
  ...
  /**
   * @description Set save-type toggle (the persona's art vs the user's art)
   * @upstream Called by: Toggle button in generation UI
   * @downstream Calls: setState (Zustand)
   * @param {boolean} asPersonaArt - True to save as the persona's art, false for the user's art
   */
  setSaveAsPersonaArt: (asPersonaArt: boolean) => set({ saveAsPersonaArt: asPersonaArt }),
  ...
  /**
   * @description Generate image from text prompt via Cloudflare Workers AI
   * (the sole image provider — included with the Workers plan, content-filtered)
   *
   * Validation:
   ...
   * - 'art_result' (the persona's creative output, saveAsPersonaArt = true)
   * - 'user_art' (the user's UI-generated art, saveAsPersonaArt = false)
   *
   * Validation:
   * - Requires generatedImage to be non-null, otherwise logs error and returns
   *
   * Image extraction:
   * - Tries generatedImage.base64 first (legacy response shape)
   * - Falls back to generatedImage.image (Cloudflare Workers AI format)
  ...
  saveGeneratedImage: async () => {
    ...
    try {
      ...
      await api.post("/save-art", {
        image: imageData,
        prompt: editedPrompt,
        type: saveAsPersonaArt ? "art_result" : "user_art",
      });
  ...
  /**
   * @description Set image as the persona's profile picture
   *
