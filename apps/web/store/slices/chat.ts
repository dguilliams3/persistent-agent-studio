 ...
 * handling image/video file attachments, and the pending-reply watch that keeps
 * the chat's waiting affordance honest.
 *
 * @upstream Called by:
...
/**
 * Reply-watch cadence. A reply is never pushed — it lands in history when the
 * persona's next cycle writes it (live) or when the scripted append fires
 * (demo). ChatView's ambient 30s poll is far too coarse to deliver it: it
 * ticks on ITS OWN mount-anchored schedule, so a reply that exists 6s after a
 * send could sit undelivered for another 30s. Hence a send-anchored watch that
 * starts fast and backs off — quick feedback for a fast backend, gentle on a
 * slow one, no mode knowledge in the store.
 */
const REPLY_POLL_START_MS = 500;
const REPLY_POLL_GROWTH = 1.7;
const REPLY_POLL_MAX_MS = 8_000;
/** Give up after this long. Matches triggerThinkNow's 4-minute watch. */
const REPLY_WAIT_BUDGET_MS = 240_000;

export interface ChatSlice {
  /**
   * True from a successful send until the persona's next entry lands (or the
   * watch times out). Drives the chat's pending affordance. Deliberately NOT
   * `isThinking`: that flag disables the composer and means "a think cycle is
   * running", which is a different claim.
   */
  isAwaitingReply: boolean;
  sendMessage: () => Promise<void>;
  watchForReply: (baselineId: number) => Promise<void>;
...
export const createChatSlice: StateCreator<AppState, [], [], ChatSlice> = (
   ...
   * Fetches updated history on success, then hands off to watchForReply —
   * that first fetch CANNOT contain the reply (it does not exist yet), so the
   * send is only half the interaction.
   *
   * @upstream Called by: ChatTab send button, Enter key handler
   * @downstream Calls: api.post('/message'), addLog, setUserInput, setSelectedImage, setImagePreview, fetchHistory, watchForReply
   *
  ...
  sendMessage: async () => {
    ...
    try {
      ...
      addLog("✅ Sent");
      await fetchHistory();
      // The reply is NOT in that response — it lands later, on the persona's
      // next cycle. Watch for it from here rather than leaving the visitor
      // staring at an unchanged thread until the ambient poll happens to tick.
      const settled = get().history;
      if (Array.isArray(settled)) {
        const baselineId = (settled as Array<{ id: number }>).reduce(
          (maxId, entry) => Math.max(maxId, entry.id),
          0,
        );
        void get().watchForReply(baselineId);
      }
    } catch (err: unknown) {
  ...
  /**
   * @description Poll history until the persona's next entry lands, holding
   * `isAwaitingReply` true for the whole wait.
   *
   * Mirrors the watch triggerThinkNow already runs for cycle output, for the
   * same reason: the reply arrives out-of-band, so resetting the pending flag
   * when the POST resolves would make the affordance lie by ~all of the wait.
   * "Next entry" = any row above the baseline that the visitor did not write —
   * a reply bubble, or a thought/action drip-down. Either way the thread
   * visibly changed, so the indicator has stopped being true and must clear.
   *
   * @upstream Called by: sendMessage (fire-and-forget — a send must not block
   *   on a reply, or the composer would hang for minutes)
   * @downstream Calls: fetchHistory
   *
   * @param baselineId - Highest entry id known BEFORE the reply (the send's
   *   own row included), so only genuinely new rows count.
   *
   * @tests apps/web/store/slices/__tests__/chat.test.js ("pending reply")
   *   - "raises isAwaitingReply after a successful send"
   *   - "leaves isAwaitingReply false when the send fails"
   *   - "holds isAwaitingReply across polls until a reply lands"
   *   - "clears on a non-bubble entry — the thread changed either way"
   *   - "ignores the visitor's own later messages"
   *   - "gives up after the wait budget instead of claiming forever"
   */
  watchForReply: async (baselineId: number) => {
    const { fetchHistory } = get();
    set({ isAwaitingReply: true });
    try {
      let delay = REPLY_POLL_START_MS;
      let waited = 0;
      while (waited < REPLY_WAIT_BUDGET_MS) {
        await new Promise((resolve) => setTimeout(resolve, delay));
        waited += delay;
        delay = Math.min(delay * REPLY_POLL_GROWTH, REPLY_POLL_MAX_MS);
        await fetchHistory();
        const entries = get().history as Array<{ id: number; type: string }>;
